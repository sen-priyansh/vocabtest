...
function DashboardContent() {
  ...
  const { deleteAllTestHistory } = useTestResults();
  ...
  const handleDeleteHistory = async () => {
    setDeleting(true);
    try {
      const success = await deleteAllTestHistory();
      if (success) {
        // Clear local state
        setRecentTests([]);
        setStats(null);
        ...
        console.error('Failed to delete test history');
        alert('Failed to delete test history. Please try again.');
      }
    } catch (error) {
      console.error('Error deleting test history:', error);
      alert('An error occurred while deleting test history. Please try again.');
    } finally {
      setDeleting(false);
    }
  };
          ...
          {activeTab === 'history' && (
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-bold text-gray-900 dark:text-white">Complete Test History</h2>
                {recentTests.length > 0 && (
                  <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6 max-w-md mx-4">
            <div className="text-center">
              ...
              <p className="text-gray-600 dark:text-gray-300 mb-6">
                Are you sure you want to delete all your test history? This action cannot be undone.
              </p>
              <div className="flex space-x-3">
                ...
                <button
                  ...
                  disabled={deleting}
                  className="flex-1 px-4 py-2 bg-red-500 hover:bg-red-600 disabled:bg-red-300 text-white rounded-lg transition-colors flex items-center justify-center"
                >
                  {deleting ? (
                    <>
